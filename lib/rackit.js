 ...
 * @param {function(Error, meta)} cb - Callback function. Recieves an error and metadata hash.
...
Rackit.prototype.getMeta = function(cloudpath, cb) {
	...
	o1._cloudRequest(options, function(err, response, body) {
		...
		for (sKey in headers) {
			if (sKey.indexOf(prefix) === 0 && headers.hasOwnProperty(sKey)) {
...
/*
 * Retrieves the metadata for a cloud file, plus the "standard" metadata
 * for the file (etag, timestamp, content-type)
 * @param {string} cloudpath - The relative path to the cloud file <container>/<file>
 * @param {function(Error, meta)} cb - Callback function. Recieves an error and metadata hash.
 * The error will be present if the specified file does not exist.
 */
Rackit.prototype.getMetaFull = function(cloudpath, cb) {
	var o1 = this;
		...
		for (sKey in headers) {
			if (headers.hasOwnProperty(sKey)) {
				if(sKey.indexOf(prefix) === 0) {
					...
					meta[prefixes[sKey]] = headers[sKey];
				}
			}
		}
