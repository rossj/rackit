...
Rackit.prototype._findServiceEndpoint = function (catalog, service, region) {
	var serviceIdx = _(catalog).pluck('type').indexOf(service);

	if (serviceIdx < 0)
		return null;

	var endpoints = catalog[serviceIdx].endpoints;

	var endpointIdx = _(endpoints).pluck('region').indexOf(region);

	if (serviceIdx < 0)
		return null;

	return endpoints[endpointIdx].publicURL;
};
...
Rackit.prototype._getContainer = function (cb) {
	...
	if (!asContainers.length) {
		...
		o1._createContainer(name, function (err) {
			cb(err, name);
		});
		return;
	}
